...
一個 Windows 置頂小工具，用中文顯示 Codex 剩餘用量。
...
- 5 小時窗口：顯示剩餘百分比、已用百分比與重置時間。
- 每週上限：顯示剩餘百分比、已用百分比與重置時間。
- 顯示方案、額外點數狀態、資料來源與更新時間。
- 按鈕：`刷新`、`重新連線`、`關閉`。
- 自動更新頻率：每 10 分鐘一次。
...
`run-widget.ps1` 使用 `uv run --no-project --python 3.13`，Python runtime 由 UV 管理，不需要專案內虛擬環境。
...
uv run --no-project --python 3.13 python -c "from codex_usage_widget.reader import read_latest_usage; print(read_latest_usage())"
```